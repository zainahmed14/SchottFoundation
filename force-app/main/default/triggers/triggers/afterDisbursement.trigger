trigger afterDisbursement on outfunds__Disbursement__c (after insert, before insert) {
   ...
   List<Id> frList = new List<Id>();
    ...
    Map<Id,outfunds__Funding_Request__c> newFrMap = new Map<Id,outfunds__Funding_Request__c>([Select Id, outfunds__FundingProgram__c, Fully_Disbursed_Date__c, outfunds__Status__c, outfunds__Awarded_Amount__c from outfunds__Funding_Request__c
                                                                                          where Id IN: frList]);
 
    
    System.debug('newFrMap: ' + newFrMap);
   
    Map<Id,outfunds__Disbursement__c> newDisMap = new Map<Id,outfunds__Disbursement__c>([Select Id, outfunds__Funding_Request__c, outfunds__Disbursement_Date__c, outfunds__Amount__c, outfunds__Funding_Request__r.outfunds__Awarded_Amount__c from outfunds__Disbursement__c 
